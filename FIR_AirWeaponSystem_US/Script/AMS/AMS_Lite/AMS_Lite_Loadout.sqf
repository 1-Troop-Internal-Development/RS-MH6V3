...
if (_turret_check != -1) then
{
	...
	private _gunner_weapons = getArray ([(typeof _plane), _gunner_turret] call BIS_fnc_turretConfig >> "weapons");
	private _gunner_mags = getArray ([(typeof _plane), _gunner_turret] call BIS_fnc_turretConfig >> "magazines");	
...
for "_i" from 0 to _plane_ary_count do
{
	...
	_element = _plane_ary select _i;
	_name = _element select 0;
	_equip = _element select 1;
	_check = _element select 2;
	if (_custom) then
	{
