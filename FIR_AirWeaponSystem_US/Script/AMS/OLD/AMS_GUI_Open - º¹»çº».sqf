...
if (_list_count == 0) ExitWith {_plane vehiclechat "No AMS supported object found within 160m."};

If (not Local _plane) ExitWith {};
if (not(_plane iskindof _AircraftBase)) ExitWith {_plane vehiclechat "You Must get in the right aircraft!";};
...
if (isNil {profileNamespace getVariable _CustomPreset}) then
{
	...
	_plane vehiclechat "custom preset data installed.(you can see this message only one time)";
}
else
{
