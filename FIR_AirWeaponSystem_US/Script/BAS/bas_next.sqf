...
private _current = _computer getVariable ["AWS_BAS_CURRENT",0];

_current = _current  + 1; 
_count = _count - 1;

if (_current > _count) exitWith {_current = _count; systemChat "last slide"};

_computer setVariable ["AWS_BAS_CURRENT",_current,true];

_current_slide = _slideshow_slide select _current;

private _slide = _current_slide param [0,"#(rgb,8,8,3)color(1,1,1,1)"];
private _type = _current_slide param [1,"photo"];
private _keep = _current_slide param [2,"yes"];
private _music = _current_slide param [3,""];
