...
{
	private _mag = _pylonlist select _i;
	private _ammoname = getText (Configfile >> "CfgMagazines" >> _mag >> "ammo" );
	...
	If (_gps_bomb == 1 or _gps_mssl == 1) then
	{
		_ammo_c = _plane ammoOnPylon (_i+1);
		// array [pylon index , current ammo count]
		_ary = [(_i+1),_ammo_c];
		_gps_array pushBack _ary;		
	};
};
hintsilent format ["%1",_gps_array];
