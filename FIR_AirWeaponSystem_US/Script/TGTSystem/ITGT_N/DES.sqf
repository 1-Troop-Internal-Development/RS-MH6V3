...
if (_pylon_cursel isEqualTo -1 or _tgt_cursel isEqualTo -1) exitWith {systemChat localize "str_FIR_ITGT_DES_SELECT_CHECK";};

_pylon_value = _lb_pylon_ctrl lbData _pylon_cursel;
_tgt_data = _lb_tgt_ctrl lbData _tgt_cursel;

//hint format ["%1 / %2",_pylon_value,_tgt_data];
...
_mkr_local = ((_tgt_array select _tgt_selected_path)select 3);

_gps_array set [_pylon_selected_path,[_pylon_code,_pylon_num,_pylon_ammo_count,_wep_name,_tgt_id_get,true,_mode]];

if (_mkr_local isEqualTo "global") then
{
	_mkr_id setMarkerColor "ColorRed";
}
else
{
	_mkr_id setMarkerColorLocal "ColorRed";
};


systemChat format [localize "str_FIR_ITGT_DES_LOCKED",_pylon_num,_tgt_id_get];
