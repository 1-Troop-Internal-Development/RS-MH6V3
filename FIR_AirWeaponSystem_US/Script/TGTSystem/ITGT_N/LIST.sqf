...
private _pylonlist = _plane getVariable "AWS_ITGT_GPS_ARRAY";
private _pylonlist_count = (count _pylonlist)-1;
private _pylon_ary = [];
// systemchat format ["%1",_pylonlist];

for "_r" from 0 to _pylonlist_count do
{
	// array [code,pylon index , current ammo count,wep_name]
	 _select_pylon = _pylonlist select _r;
	 _index = _select_pylon select 0;
	 _name = _select_pylon select 3;
	
	 _txt = format ["p:%1/%2",_index,_name];
	
	 _pylon_ary_elements = [_txt,_index];
	_pylon_ary pushback _pylon_ary_elements;			
};

[5581,1500,_pylon_ary,"data"] call FIR_fnc_lbadd_display;

// [5581,1500,_pylon_ary,"value"] call FIR_fnc_lbadd_display;

private _tgtlist = _plane getVariable "AWS_ITGT_GPS_TGTLIST";
...
// element structure [index,tgtID,tgtpos,markerID];
	 _select_tgt = _tgtlist select _r;
	 _id = _select_tgt select 1;
	 _pos = _select_tgt select 2;
	
	 _txt = format ["%1/%2",_id,_pos];
	
	 _tgt_ary_elements = [_txt,_id];
