...
_pylon_list = _plane getVariable ["AWS_ITGT_PYLON_ORDERLIST","no"];
...
if (_checked) then
{
	...
	if (_pylon_list isEqualTo "no") then 
	{
		...
		_plane setVariable ["AWS_ITGT_PYLON_ORDERLIST",_pylonlist];
	}
	else 
	{
		_pylonlist = _pylon_list;
		_hp_num = _hp_num - 1;
		_pylonlist set [_hp_num,_text];
		hint format ["%1",_pylonlist];
		_plane setPylonsPriority _pylonlist;		
