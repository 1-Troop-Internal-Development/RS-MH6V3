...
private _sub = "FIR_WP_Airburst" createvehicle _wp_pos;
...
private _manlist = _subpos nearEntities ["Man", 45];
private _manlist_c = count _manlist;

for [{_i=0}, {_i < _manlist_c}, {_i=_i+1}] do
{
	_victim = _manlist select _i;
	_scr = [_victim] execVM "FIR_AirWeaponSystem_US\Script\WP_Dam.sqf";
};
...
// _smokez = [_subpos,45] execVM "FIR_AirWeaponSystem_US\Script\WP_Dam_Zone.sqf";
