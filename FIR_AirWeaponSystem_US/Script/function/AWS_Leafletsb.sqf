...
switch _mode do {
	case "fired": {
		params ["_obj","_weapon","_muzzle","_mode","_ammo","_magazine","_projectile"];

		_missobj = nearestobject[_obj, _ammo];
		...
		if (count _leafletsParams > 0) then {
			...
			// (_leafletsParams select 0) set [4,60];
			//--- Use custom model
			...
			_leaflets = "#particlesource" createVehicleLocal position _missobj;
			...
			_leaflets attachto [_missobj,[0,0,0]];
		};
